...
Application full-stack de gestion des réclamations avec Spring Boot 3.3.6 et Angular 17.
...
- Suivi du statut (OUVERT, EN_COURS, RESOLU, CLOS, ESCALADE)
...
- Traitement / Escalade / Réouverture / Clôture
- Historique détaillé
...
- Export CSV
- Statistiques avancées (par statut, type, priorité, délai moyen)
...
- `GET /api/tickets/mes-assignations`
- `GET /api/tickets/all`
- `GET /api/tickets/{id}`
...
TICKETING-PROJECT/
├── ticketing-backend/
│   ├── src/main/java/com/justice/ticketing/
│   │   ├── config/
│   │   ├── controller/
...
- Rôles hiérarchisés (Citoyen, Agents, Admin)
- Guards côté frontend (authGuard, roleGuard)
...
|------|-------------------------|
| ROLE_CITOYEN | Créer / voir ses tickets |
| ROLE_AGENT_TRAITEMENT | + traitement basique |
| ROLE_AGENT_SUPPORT | + escalade, réouverture, clôture, vue globale |
| ROLE_ADMIN_SUPPORT | + gestion utilisateurs, export, stats |
...
Accès: http://localhost:4200 — Login: admin@justice.gov / Admin@123

## 📝 Notes
- Version Spring Boot mise à jour 3.3.6
- DTOs étendus & endpoints supplémentaires
- Export CSV & statistiques temps réel
